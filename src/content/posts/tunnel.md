...
Cloudflare Tunnel 的确很好用，尤其适合内网穿透、个人服务暴露和轻量站点部署。  
...
只要小黄云开启，流量实际仍然会经过 Cloudflare，这样就可以满足后续 SaaS 的回源要求。
...
例如当你配置了多个优选时，只要统一通过 `y.cf.com` 这一层来处理，后续更换优选域名时，只需要修改 `y.cf.com` 的解析记录，而不必直接改动主域名配置。
